...
export const checkLocationPermissions = async (): Promise<{granted: boolean, message?: string}> => {
  ...
  if (permissionRequestInProgress) {
    return { granted: false, message: 'Esperando respuesta de permisos...' };
  }
  ...
  try {
    ...
    console.error('Error en checkLocationPermissions:', error);
    permissionRequestInProgress = false;
    return { 
      granted: false, 
      message: 'Error al verificar los permisos. Por favor, recarga la página e inténtalo de nuevo.' 
    };
  }
...
export const requestLocationPermissions = async (): Promise<{granted: boolean, message?: string}> => {
  ...
  return new Promise((resolve) => {
    ...
    }, 30000); // Aumentado a 30 segundos para dar más tiempo al usuario
