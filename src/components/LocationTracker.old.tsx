...
export const LocationTracker: React.FC<LocationTrackerProps> = ({ tripId, onDistanceUpdate = () => {} }) => {
  ...
  return (
    <div className="location-tracker">
      ...
      <div className="flex items-center space-x-2">
        ...
        {isCheckingLocation && <span className="text-sm text-muted-foreground">Verificando ubicación...</span>}
      </div>
  ...
  useEffect(() => {
    const verifyLocation = async () => {
      const { hasPermission, isEnabled } = await verifyLocationStatus();
      if (hasPermission && isEnabled) {
        startLocationTracking();
      }
    };
  ...
  // Efecto para manejar cambios de visibilidad
  useEffect(() => {
